...
const Plugin = () => {
  ...
  function getSlidifyOptions(options) {
    options = options || {}
    options.separator =
      options.separator || DEFAULT_SLIDE_SEPARATOR
  ...
  function slidify(markdown, options) {
    ...
    for (
      var i = 0, len = sectionStack.length;
      i < len;
      i++
    ) {
      // vertical
              ...
              loadExternalMarkdown(section).then(
                ...
                function (xhr, url) {
                  section.outerHTML = slidify(
                    xhr.responseText,
                    {
                      separator: section.getAttribute(
                        "data-separator"
                      ),
                      verticalSeparator:
  ...
  function loadExternalMarkdown(section) {
    return new Promise(function (resolve, reject) {
      ...
      var datacharset = section.getAttribute(
        "data-charset"
      )
   ...
   */
  function addAttributeInElement(
    node,
    elementTarget,
    separator
  ) {
    var mardownClassesInElementsRegex = new RegExp(
      separator,
      "mg"
    )
    ...
    if (
      (matches =
        mardownClassesInElementsRegex.exec(nodeValue))
    ) {
      ...
      while (
        (matchesClass =
          mardownClassRegex.exec(classes))
      ) {
        if (matchesClass[2]) {
  ...
  function addAttributes(
    ...
    if (
      ...
      for (
        ...
        if (
          typeof childElement.setAttribute ==
            "function" ||
        ...
        renderer.code = (code, language) => {
          ...
          if (CODE_LINE_NUMBER_REGEX.test(language)) {
            lineNumbers = language
              .match(CODE_LINE_NUMBER_REGEX)[1]
              .trim()
