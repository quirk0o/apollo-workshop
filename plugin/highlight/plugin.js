...
!(function (r, o) {
  ...
  function u(e) {
    for (
      var n = e.toString(), t = e.anchorNode;
      "TD" !== t.nodeName;
  ...
  function n(e) {
    try {
      var n = o.querySelectorAll(
        "code.hljs,code.nohighlight"
      )
      for (var t in n)
        n.hasOwnProperty(t) &&
          (n[t].classList.contains("nohljsln") ||
            d(n[t], e))
    } catch (e) {
    ...
    return (
      ...
      (function (e, n) {
        ...
        if (1 < t.length || n.singleLine) {
          ...
          return p('<table class="{0}">{1}</table>', [
            i,
            r,
          ])
        }
...
const Plugin = {
        ...
        highlightSteps
          ...
          .forEach(function (highlight) {
            ...
            fragmentBlock.setAttribute(
              "data-line-numbers",
              Plugin.serializeHighlightSteps([
                highlight,
              ])
            )
  ...
  scrollHighlightedLineIntoView: function (
    ...
    if (scrollState.currentBlock) {
      block.scrollTop =
        scrollState.currentBlock.scrollTop
    }
    ...
    var highlightBounds =
      this.getHighlightedLineBounds(block)
  ...
  getHighlightedLineBounds: function (block) {
    var highlightedLines = block.querySelectorAll(
      ".highlight-line"
    )
    if (highlightedLines.length === 0) {
      ...
      return {
        ...
        bottom:
          lastHighlight.offsetTop +
          lastHighlight.offsetHeight,
      }
    }
  },
    ...
    if (highlightSteps.length) {
      highlightSteps[0].forEach(function (highlight) {
        ...
        if (elementsToHighlight.length) {
          elementsToHighlight.forEach(function (
            lineElement
          ) {
   ...
   * // ]
   */
  deserializeHighlightSteps: function (
    highlightSteps
  ) {
    // Remove whitespace
    ...
    highlightSteps = highlightSteps.split(
      Plugin.HIGHLIGHT_STEP_DELIMITER
    )
