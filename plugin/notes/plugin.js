...
const Plugin = () => {
  ...
  function callRevealApi(
    ...
    let result = deck[methodName].apply(
      deck,
      methodArguments
    )
    speakerWindow.postMessage(
  ...
  function post(event) {
    let slideElement = deck.getCurrentSlide(),
      notesElement =
        slideElement.querySelector("aside.notes"),
    ...
    if (slideElement.hasAttribute("data-notes")) {
      messageData.notes =
        slideElement.getAttribute("data-notes")
    ...
    if (fragmentElement) {
      ...
      if (fragmentNotes) {
        ...
        fragmentElement.hasAttribute("data-notes")
      ) {
        messageData.notes =
    ...
    if (notesElement) {
      ...
      messageData.markdown =
        typeof notesElement.getAttribute(
          "data-markdown"
        ) === "string"
    }
